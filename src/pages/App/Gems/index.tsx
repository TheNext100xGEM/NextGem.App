...
const FilterDrop = ({ name, right, children, className }: PropsFilterDrop) => {
  return (
    <Dropdown 
      className={className}
      opener={
        <Button color="tertiary">{name} {right && <small>{right}</small>}</Button>
      }
    >
      {children}
    </Dropdown>
...
const FilterByCategories = () => {
  const countTotalAreas = (): number => CryptoMarketAreas.reduce((total, category) => total + category.list.length, 0)
...
const Filter = () => {
  ...
  return (
    <div className={classNames('filter', open && 'open')}>
      <Input icon="carbon:search" type="search" placeholder="Research a project or token..." />
      <div className="filter-actions">
        <FilterBySort />
        <FilterAiNote />
        <FilterByCategories />
        <FilterByChains />
        <Button className="save" icon="carbon:checkmark-outline" onClick={() => setOpen(false)} status="success">Save filter</Button>
      </div>
      <Button className="filter-opener" minus icon="carbon:list-boxes" onClick={() => setOpen(true)} title="Filter"/>
    </div>
  )
}
